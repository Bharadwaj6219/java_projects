...
//		File nodes = new File(absolutePath + "\\src\\nodes.config");
		...
		try {
			...
			Site mySite = new Site(numberOfNodes, thisSiteNumber, hasToken, ipAddress, port);

			// Open a socket
			ListenToBroadcast listenToBroadcast = new ListenToBroadcast(mySite, port[thisSiteNumber - 1]);
	...
	public static void exitCS(Site mySite, int thisSiteNumber, int numberOfNodes, String[] ipAddress, int[] port, int numOfSites) {

		mySite.LN[thisSiteNumber - 1] = mySite.RN[thisSiteNumber - 1];

		// Send updated LN value to all sites
		String message = "ln," + thisSiteNumber + "," + mySite.LN[thisSiteNumber - 1];
		...
		for (int i = 0; i < numberOfNodes; i++) {
			if (mySite.RN[i] == mySite.LN[i] + 1) {
				if (!mySite.tokenQueue.contains(i + 1)) {
					mySite.tokenQueue.add(i + 1);
				}
			}
		}

		if (mySite.tokenQueue.size() > 0) {
			mySite.sendToken(mySite.tokenQueue.poll());
		}
	}
}